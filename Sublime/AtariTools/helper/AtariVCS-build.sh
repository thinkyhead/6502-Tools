...
[[ $INSRC =~ \.(s|asm|inc|6502)$ ]] || { echo "Not Valid source file" ;  exit 1 ; }
...
if [[ $EXTRA == "run" ]]; then
  ...
  if [[ -d "$z26" || -d "$STELLA" ]]; then
    ...
    #open -a Stella "$OUTFILE" --args -tv.phosblend 20 -tv.filter 1 -tv.scanlines 40 -tv.scanmask thin -display 2
  else
